/**
 * @fileoverview Seeds a local database with a superadmin, a regular user,
 * and one organization on the free plan.
 *
...
export async function seedDatabase(
  ...
  await database.transaction(async tx => {
    ...
    const organizationId = organizationsTable.id.defaultFn!()
    ...
    await tx.insert(organizationSubscriptionsTable).values({ organizationId })
